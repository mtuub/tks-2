...
def uploadVideo(session_id, video, title, tags, schedule_time=0, verbose=True):
    ...
    signature = AWSsignature(access_key, secret_key,
    ...
    for tag in tags:
        ...
        try:
            ...
            verified_tag = tag
        text += " #"+verified_tag
        text_extra.append({"start": len(text)-len(verified_tag)-1, "end": len(text),
                           "user_id": "", "type": 1, "hashtag_name": verified_tag})
...
if __name__ == "__main__":
    ...
    uploadVideo(args.session_id, args.path, args.title,
                args.tags, args.schedule_time, verbose=True)
